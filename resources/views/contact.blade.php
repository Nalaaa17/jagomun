...
<main>
    ...
    <div class="pb-16">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if (session('success'))
                <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)" class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 mb-6 rounded-md shadow-md" role="alert">
                    <p>{{ session('success') }}</p>
                </div>
            @endif
                    ...
                    <form action="{{ route('contact.submit') }}" method="POST">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full form-input-custom" required />
                                <x-input-error :messages="$errors->get('name')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="email" :value="__('Email')" />
                                <x-text-input id="email" name="email" type="email" class="mt-1 block w-full form-input-custom" required />
                                <x-input-error :messages="$errors->get('email')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="message" :value="__('Your Message')" />
                                <textarea id="message" name="message" rows="5" class="mt-1 block w-full form-textarea-custom border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required></textarea>
                ...
                <div class="p-8 sm:p-10 bg-[#1E2233] text-[#F2EFEA]">
                    ...
                    <div class="space-y-6">
                        <div class="flex items-start">
                            <div class="flex-shrink-0 mt-1"><svg class="h-6 w-6 text-[#B4976B]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg></div>
                            <div class="ml-4">
                                <h3 class="text-lg font-medium text-white">Address</h3>
                                <p class="mt-1 text-[#D6C4A4]">Jl. Kalimantan, Krajan Timur, Sumbersari, Jember</p>
                            </div>
                        </div>
                        <div class="flex items-start">
                            <div class="flex-shrink-0 mt-1"><svg class="h-6 w-6 text-[#B4976B]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg></div>
                            <div class="ml-4">
                                <h3 class="text-lg font-medium text-white">Email</h3>
                                <p class="mt-1 text-[#D6C4A4]">info@jagomun.com</p>
                            </div>
                        </div>
                        <div class="flex items-start">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="mt-1 text-[#D6C4A4]">+62 812-3456-7890</p>
                            </div>
                        </div>
                    </div>
                    <div class="mt-10">
