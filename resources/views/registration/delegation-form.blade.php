...
<div class="bg-slate-50 min-h-screen">
    <div class="max-w-5xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-10 px-4">
            <div id="stepper" class="flex items-center max-w-2xl mx-auto">
                ...
                <div class="stepper-item active">
                    <div class="step-counter"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h6M9 11.25h6M9 15.75h6M9 20.25h6" /></svg></div>
                </div>
                <div class="flex-auto border-t-2 transition duration-500 ease-in-out border-gray-300"></div>
                {{-- Step 2: Delegates --}}
                <div class="stepper-item">
                    <div class="step-counter"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m-7.5-2.962A3.75 3.75 0 0115 9.75v6.038c0 .621-.504 1.125-1.125 1.125H4.125A1.125 1.125 0 013 15.788V9.75A3.75 3.75 0 016.75 6h.008a3.75 3.75 0 013.742 3.75z" /></svg></div>
                </div>
                <div class="flex-auto border-t-2 transition duration-500 ease-in-out border-gray-300"></div>
                {{-- Step 3: Payment --}}
                <div class="stepper-item">
        ...
        <div class="bg-white shadow-2xl shadow-slate-300/20 rounded-xl p-8">
            ...
            <form id="delegation-form" action="{{ route('registration.delegationSubmit') }}" method="POST" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="step-content">
                        ...
                        <div class="border border-slate-200 p-6 rounded-lg">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                                ...
                                <div>
                                    <p class="block font-medium text-sm text-gray-700 mb-3">1. Select Package Type for Delegation:</p>
                                    <div id="package-selector" class="space-y-3">
                                        <label for="package_type_full" class="flex items-center cursor-pointer p-3 border rounded-md has-[:checked]:bg-indigo-50 has-[:checked]:border-indigo-500">
                                            <input type="radio" id="package_type_full" name="package_type" value="Full Accommodation" class="h-4 w-4 text-indigo-600" required>
                                            <span class="ml-3 text-sm text-gray-800">Full Accommodation</span>
                                        </label>
                                        <label for="package_type_non" class="flex items-center cursor-pointer p-3 border rounded-md has-[:checked]:bg-indigo-50 has-[:checked]:border-indigo-500">
                                            <input type="radio" id="package_type_non" name="package_type" value="Non-Accommodation" class="h-4 w-4 text-indigo-600" required>
                                            <span class="ml-3 text-sm text-gray-800">Non-Accommodation</span>
                                        </label>
                                        <label for="package_type_online" class="flex items-center cursor-pointer p-3 border rounded-md has-[:checked]:bg-indigo-50 has-[:checked]:border-indigo-500">
                                            <input type="radio" id="package_type_online" name="package_type" value="Online" class="h-4 w-4 text-indigo-600" required>
                                ...
                                <div>
                                    <p class="block font-medium text-sm text-gray-700 mb-3">2. How many delegates in your group?</p>
                                    <div id="delegate-selector" class="space-y-3">
                                        @foreach ([2, 3, 4, 5] as $count)
                                            <label for="delegate_count_{{ $count }}" class="flex items-center cursor-pointer p-3 border rounded-md has-[:checked]:bg-indigo-50 has-[:checked]:border-indigo-500">
                                                <input type="radio" id="delegate_count_{{ $count }}" name="delegate_count" value="{{ $count }}" class="h-4 w-4 text-indigo-600" required>
                                                <span class="ml-3 text-sm text-gray-800">{{ $count }} delegates</span>
                                            </label>
                                        @endforeach
                                    </div>
                                </div>
                ...
                <div id="step-2" class="form-step">
                    <div class="step-content">
                        <h2 class="text-2xl font-semibold text-slate-700 mb-6 border-l-4 border-indigo-500 pl-4">Delegate Details</h2>
                        <p class="text-slate-500 mb-6 -mt-4 pl-5">Please fill in the details for each delegate below.</p>
                ...
                <div id="step-3" class="form-step">
                    <div class="step-content">
                        <h2 class="text-2xl font-semibold text-slate-700 mb-6 border-l-4 border-indigo-500 pl-4">Payment</h2>

                        {{-- Dynamic Price Display --}}
                        <div id="total-payment-display" class="bg-indigo-50 border-l-4 border-indigo-500 text-indigo-900 p-4 rounded-r-lg mb-6 hidden" role="alert">
                        ...
                        <div class="my-6">
                            ...
                            <div class="relative">
                                <x-text-input id="referral_code" name="referral_code" type="text" class="mt-1 block w-full md:w-1/2 pr-10" placeholder="Enter delegation discount code" />
                                <div id="referral-loading" class="hidden absolute inset-y-0 right-0 pr-3 flex items-center" style="top: 50%; transform: translateY(-50%); right: 0.75rem;">
                        ...
                        <div class="text-sm text-slate-600 p-4 bg-slate-100 rounded-lg mb-6 border border-slate-200">
                            <p class="mb-2">Please refer to JAGOMUN Official Instagram @jagomun.2025 for the prices. Transfer the registration payment to:</p>
                        ...
                        <div>
                            ...
                            <label for="payment_proof" class="flex items-center justify-center border-2 border-dashed border-gray-300 rounded-lg p-6 cursor-pointer hover:border-indigo-500 hover:bg-slate-50 transition duration-150 ease-in-out">
                ...
                <div class="flex items-center justify-between mt-12 pt-6 border-t border-slate-200">
                    ...
                    <button type="submit" id="submit-btn" class="hidden px-8 py-3 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 transition-colors">
...
<template id="delegate-template">
    <div class="delegate-form-block border border-slate-200 p-6 rounded-lg bg-white relative shadow-md shadow-slate-200/50">
        <h3 class="delegate-title text-xl font-semibold text-slate-800 mb-6 border-b border-slate-200 pb-4">Delegate #</h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            ...
            {{-- Experience --}}
            <h4 class="md:col-span-2 text-lg font-semibold text-slate-700 border-b pb-2 mt-4">Experience & Documents</h4>
            <div class="md:col-span-2">
            ...
            <div>
                ...
                <input name="delegates[__INDEX__][student_id]" type="file" class="input-file-basic" accept=".pdf" required />
            </div>
            <div class="md:col-span-1">
                <x-input-label for="parental_consent" :value="__('Parental Consent Letter (PDF - If applicable)')" />
                <p class="text-sm text-slate-500 mb-1">Required for applicants under 18 attending offline. Template: <a href="http://bit.ly/ParentalConsentLetterJAGOMUN2025" target="_blank" class="text-indigo-600 hover:underline font-semibold">Download here</a></p>
             ...
             <div class="md:col-span-2">
                ...
                <p class="font-medium text-sm text-gray-700">Social Media Upload <a href="https://www.instagram.com/jagomun.2025" target="_blank" class="text-indigo-600 hover:underline">@jagomun.2025</a></p>
                <input name="delegates[__INDEX__][social_media_proof_path]" type="file" class="input-file-basic" accept=".jpg,.jpeg,.png,.pdf" />
            </div>
            ...
            <div class="md:col-span-2 space-y-3">
                ...
                <label class="flex items-start cursor-pointer">
                    ...
                    <span class="ml-3 text-sm text-gray-700">I agree to allow the organizing committee to use my personal data for administrative and event-related purposes.</span>
                </label>
            </div>
        </div>
    </div>
...
<style>
    .stepper-item { @apply flex items-center; }
    .stepper-item .step-counter { @apply relative flex items-center justify-center w-10 h-10 rounded-full font-semibold transition duration-500 ease-in-out bg-slate-200 text-slate-500 flex-shrink-0; }
    .stepper-item .step-name { @apply ml-3 text-sm font-medium text-slate-500; }
    .stepper-item.completed .step-counter, .stepper-item.active .step-counter { @apply bg-indigo-600 text-white; }
    .stepper-item.completed .step-name, .stepper-item.active .step-name { @apply text-slate-800; }
    #stepper > .border-t-2 { @apply mx-4; }
    .stepper-item.completed + .border-t-2, .stepper-item.active + .border-t-2 { @apply border-indigo-600; }
    .form-step { display: none; }
    .form-step.active-step { display: block; }

    /* Basic styles for inputs inside template to avoid repeating long class lists */
    .input-file-basic { @apply block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none mt-1; }
    .input-select-basic { @apply border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full; }
    .input-textarea-basic { @apply border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full; }
</style>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function checkReferralCode() {
        ...
        try {
            ...
            if (response.ok && data.valid) {
                ...
                referralStatusEl.innerHTML = `<span class="font-semibold text-green-600">✅ ${data.message}</span>`;
            } else {
                currentDiscount = 0;
                referralStatusEl.innerHTML = `<span class="font-semibold text-red-600">❌ ${data.message || 'Invalid code.'}</span>`;
            }
        } catch (error) {
            currentDiscount = 0;
            referralStatusEl.innerHTML = `<span class="font-semibold text-red-600">❌ Error connecting to server.</span>`;
        } finally {
    ...
    const updateAndRender = () => {
        ...
        if (selectedPackage && selectedCount) {
            ...
            container.innerHTML = `<div class="text-center text-slate-500 bg-slate-100 p-6 rounded-lg">Please select a package and number of delegates in Step 1 to see the forms here.</div>`;
        }
        updatePrice();
    };
    ...
    function setupFileUpload(inputId, textId) {
        ...
        if (input && text) {
            input.addEventListener('change', () => {
                text.textContent = input.files.length > 0 ? input.files[0].name : 'Click to upload file. Max 2MB.';
            });
        }
    }
    setupFileUpload('payment_proof', 'payment_proof_filename');

    if(oldPackageType) document.querySelector(`input[name="package_type"][value="${oldPackageType}"]`).checked = true;
    if(oldDelegateCount) document.querySelector(`input[name="delegate_count"][value="${oldDelegateCount}"]`).checked = true;
    updateAndRender();
});
</script>
@endsection
